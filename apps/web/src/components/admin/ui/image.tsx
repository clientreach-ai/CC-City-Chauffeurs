...
import { deleteMedia, getMedia, toImageRef, uploadMedia } from "@/lib/cms/repositories/media";
import { ACCEPTED_IMAGE_TYPES, mediaStorage } from "@/lib/cms/storage";
 ...
 * several hundred kilobytes each at source. Local previews are data URLs and
...
function OriginNote({ src }: { src: string }) {
  return src.startsWith("data:") ? (
    <span className="text-[0.75rem] text-white/55">Local preview — not uploaded</span>
  ) : (
    <span className="text-[0.75rem] text-white/55">Site photography</span>
  );
}
...
export function MediaLibraryDialog({
  ...
  return (
    <Dialog
      ...
      description={
        <>
          The site&rsquo;s own photography, plus anything you add here. Added images are{" "}
          <strong className="font-medium text-white">not uploaded</strong> — {mediaStorage.description.toLowerCase()}
        </>
      }
    >
            ...
            onFiles={async (files) => {
              ...
              if (added.length) {
                ...
                notify.success(
                  ...
                  "Kept in this browser as a preview — not uploaded.",
                );
              }
            }}
          >
